...
# --- راه‌اندازی کلاینت OpenAI برای اتصال به AvalAI (با تنظیم Timeout) ---
client = None
if AVALAI_API_KEY:
    try:
        client = OpenAI(
            ...
            timeout=20.0 # افزایش زمان‌بندی برای پایداری شبکه
        )
...
async def set_role(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ### اصلاحیه نهایی System Prompt برای روایت دوم شخص و معرفی شخصیت‌ها ###
    system_prompt = (
        "تو یک نقال حماسی و باوفا به شاهنامه هستی. "
        "داستان را به صورت یک داستان‌گوی حرفه‌ای و با **ادبیاتی ساده، روان و کاملاً داستان‌گویانه (نه سنگین و قدیمی)** روایت کن. "
        "**روایت را کاملاً در ضمیر 'تو' (دوم شخص مفرد) انجام بده تا مخاطب حس کند که در بطن داستان است.** "
        "**هر شخصیت جدیدی که وارد داستان می‌شود (مثلاً 'گودرز') را بلافاصله در پرانتز معرفی کن (مثلاً: گودرز، پهلوانی دلیر).** "
        "**هر بخش داستان باید بسیار مختصر باشد (حداکثر ۳ جمله).** "
        "**پس از اتمام داستان، حتماً یک تصمیم مستقیم (مثلاً: فرار کنی، بجنگی، کمک بخواهی؟) از مخاطب بپرس. سؤال نباید در مورد اتفاق باشد. ۳ گزینه انتخابی جدید برای ادامه داستان به صورت [1. متن گزینه] [2. متن گزینه] [3. متن گزینه] ارائه کن. متن داخل براکت‌ها باید بسیار کوتاه (حداکثر ۵ کلمه) باشد.**"
    )
    
    first_prompt = f"داستان من را به عنوان «{user_role}» آغاز کن. اولین صحنه و اولین تصمیم/انتخاب پیش روی من چه خواهد بود؟"
